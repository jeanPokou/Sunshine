...
class ForecastAdapter( private  var mWeatherData:Array<String>?,private  var mOnClickListener: ListItemClickedListener): RecyclerView.Adapter<ForecastAdapter.ForecastViewHolder>(){
    ...
    inner class  ForecastViewHolder (itemView : View, private val mWeatherTextView: TextView = itemView.find<TextView>(R.id.tv_weather_data)) : RecyclerView.ViewHolder(itemView) {
        fun bind( position: Int) {
            with(mWeatherTextView) {
                ...
                setOnClickListener {
                    mWeatherData?.get(position)?.let {
                        mOnClickListener.onListItemClicked(it)
                    }
                }
            }
        }
